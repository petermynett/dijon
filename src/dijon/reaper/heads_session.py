"""Reaper heads session generation and reading."""
...
from dijon.global_config import AUDIO_HEADS_DIR, PROJECT_ROOT, RAW_AUDIO_DIR
...
def create_heads_session(
    audio_file: Path,
    *,
    dry_run: bool = False,
    open_session: bool = False,
) -> dict:
    """Create a new Reaper heads session from an audio file.
    ...
    session_path = HEADS_DIR / f"{audio_file.stem}_heads.RPP"
    ...
    # Check for existing heads annotation file
    annotation_file = AUDIO_HEADS_DIR / f"{audio_file.stem}_heads.json"
    heads_data = None
    ...
    if annotation_file.exists():
        try:
            ...
            if "entries" not in annotation_json:
                ...
                if entries:
                    heads_data = entries[0]
        except (json.JSONDecodeError, KeyError) as e:
            # If we can't parse, continue without heads
            pass
    ...
    # Insert markers if we have heads data
    if heads_data and heads_data.get("markers"):
        marker_lines = _generate_marker_lines(heads_data["markers"])
    ...
    # Create heads directory if needed
...
def read_heads(rpp_file: Path) -> dict:
    """Read heads data from a Reaper project file.
    ...
    # Example: MARKER 1 0.17 head in 8 0 1 B {4B33297A-385B-D84C-8CD4-EB377E55CE19} 0
    pattern = (
    ...
    AUDIO_HEADS_DIR.mkdir(parents=True, exist_ok=True)

    # Normalize filename: replace -heads with _heads, ensure _heads suffix
    stem = rpp_file.stem
    # Replace -heads with _heads if present
    if stem.endswith("-heads"):
        stem = stem[:-6] + "_heads"
    elif not stem.endswith("_heads"):
        # If it doesn't end with _heads, add it
        stem = stem + "_heads"
    
    output_file = AUDIO_HEADS_DIR / f"{stem}.json"
...
def order_heads_in_file(file_path: Path) -> dict:
    """Order heads in a single JSON file by position and renumber sequentially.
    
    Reads a heads JSON file, processes each entry to order markers by position
    and renumber them sequentially, then writes the file back with the same
    formatting style.
    
    Args:
        file_path: Path to the heads JSON file to process.
    ...
    if not file_path.exists():
        raise FileNotFoundError(f"Heads file not found: {file_path}")
    ...
    # Format JSON output (same style as read_heads)
    formatted_entries = []
...
def order_all_head_files() -> dict:
    """Order heads in all JSON files in the audio-heads directory.
    
    Finds all *_heads.json files in AUDIO_HEADS_DIR, processes each file
    ...
    """
    # Find all heads JSON files
    head_files = sorted([
        f for f in AUDIO_HEADS_DIR.glob("*_heads.json")
        if f.is_file()
    ])
    ...
    if not head_files:
        return {
            ...
            "processed": 0,
            "files": [],
            "message": "No heads files found in audio-heads directory",
        }
    
    results = []
    for head_file in head_files:
        try:
            result = order_heads_in_file(head_file)
            results.append({
                "file": result["file"],
    ...
    return {
        "success": True,
        "processed": len(head_files),
        "files": results,
    }


def read_all_heads() -> dict:
    """Read heads data from all RPP files in the heads directory.

    Searches reaper/heads for *.RPP files, extracts markers from each,
    and writes JSON output files to data/audio-heads (prepending new entries
    if files already exist, preserving historical data).

    Returns:
        Dictionary with processing results for all files.
    """
    # Find all RPP files in heads directory (excluding backups)
    rpp_files = sorted([
        f for f in HEADS_DIR.glob("*.RPP")
    ...
    if not rpp_files:
        return {
            ...
            "processed": 0,
            "files": [],
            "message": "No RPP files found in heads directory",
        }

    results = []
    for rpp_file in rpp_files:
        try:
            result = read_heads(rpp_file)
            results.append({
